...
class GroupRepositoryImpl implements GroupRepository {
  ...
  Future<bool> canDeleteGroup(String id) async {
    ...
    final List<Map<String, dynamic>> result = await db.rawQuery(
      '''
      SELECT status, 
             (SELECT COUNT(*) FROM enrollments WHERE group_id = ?) as student_count
      FROM groups 
      WHERE id = ?
    ''',
      [id, id],
    ...
    final int studentCount = result.first['student_count'];

    // Can delete if archived OR no students
    return status == 'archived' || studentCount == 0;
  }
  ...
  Future<void> deleteGroup(String id) async {
    ...
    if (await canDeleteGroup(id)) {
      ...
      throw Exception(
        'No se puede eliminar un curso con alumnos activos. Primero debe archivar el curso.',
      );
    }
  }
}
